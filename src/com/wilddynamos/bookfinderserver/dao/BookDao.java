...
/**
 * Map between Book and the table book
 * 
 * @author JunqiWang
 * 
 */
public class BookDao extends BaseDao<Book> {
	...
	public int add(Book book) {
		try {
			return stmt
					.executeUpdate("INSERT INTO book(id, name, price, per, available_time, likes,"
	...
	public int removeById(Integer id) {
		try {
			return stmt
					.executeUpdate("DELETE FROM book WHERE id='" + id + "';");
		} catch (Exception e) {
			e.printStackTrace();
			return -1;
		}
	...
	public int removeByProp(String name, String value, String order,
			Integer limit) {
		try {
			return stmt.executeUpdate("DELETE FROM book WHERE " + name + " = '"
					+ value + "' ORDER BY " + order + " LIMIT " + limit + ";");
		} catch (Exception e) {
			e.printStackTrace();
			return -1;
		}
	...
	public int update(Book book) {
		try {
			return stmt.executeUpdate("UPDATE book SET " + "name = '"
					+ book.getName()
					+ "', "
					+ "price = "
					+ book.getPrice()
					...
					+ "', "
					+ "post_time = '"
					+ new SimpleDateFormat(DATE_TIME_FORMAT).format(book
							.getPostTime()) + "', " + "cover_path = '"
					+ book.getCoverPath() + "', " + "owner_id = "
					+ book.getOwnerId() + " " + "WHERE id = " + book.getId()
					+ ";");
		} catch (Exception e) {
			e.printStackTrace();
			return -1;
	...
	public List<Book> findAll(String order, Integer currentPage,
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT * FROM book ORDER BY "
					+ order + " LIMIT "
			...
			while (rs.next())
				books.add(new Book(rs.getInt(1), rs.getString(2), rs.getInt(3),
						rs.getBoolean(4), rs.getInt(5), rs.getInt(6), rs
								.getBoolean(7), rs.getBoolean(8), rs
								.getString(9), rs.getDate(10),
	...
	public List<Book> findAllAvailableBooks(Integer currentPage,
		...
		List<Book> books = new ArrayList<Book>();
		try {
			String sql = "SELECT * FROM book WHERE status = 0 AND owner_id <> '"
					+ userId + "'";
			if (sOrR != null)
				sql += " AND s_or_r = " + sOrR;
