...
/**
 * Map between User and the table user
 * 
 * @author JunqiWang
 * 
 */
public class UserDao extends BaseDao<User> {
	...
	public int add(User user) {
		try {
			return stmt
					.executeUpdate("INSERT INTO user(id, email, password, name, "
							...
							+ user.getGender()
							+ ", "
							+ (user.getCampus() == null ? null : "'"
									+ user.getCampus() + "'")
							+ ", "
							+ (user.getContact() == null ? null : "'"
									+ user.getCampus() + "'")
							+ ", "
	...
	public int removeByProp(String name, String value, String order,
			Integer limit) {
		try {
			return stmt.executeUpdate("DELETE FROM user WHERE " + name + " = '"
	...
	public int update(User user) {
		try {
			return stmt.executeUpdate("UPDATE user SET " + "email = '"
					+ user.getEmail()
					+ "', "
					+ "password = '"
					+ user.getPassword()
					...
					+ ", "
					+ "photo_path = "
					+ (user.getPhotoPath() == null ? null : "'"
							+ user.getPhotoPath() + "'") + " " + "WHERE id = "
					+ user.getId() + ";");
	...
	public List<User> findAll(String order, Integer currentPage,
			Integer pageSize) {
		List<User> users = new ArrayList<User>();
		try {
			pageSize = (pageSize == null ? DEFAULT_USER_PAGESIZE : pageSize);
			ResultSet rs = stmt.executeQuery("SELECT * FROM user ORDER BY "
					+ order + " LIMIT "
